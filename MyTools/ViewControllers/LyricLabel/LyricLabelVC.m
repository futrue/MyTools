...
- (void)createHollowPutLabel
{
    ...
    _hollowBkView.center = CGPointMake(SCREEN_WIDTH / 2., CGRectGetMaxY(_progressLabel2.frame) + 100);
    ...
    start.center = CGPointMake(SCREEN_WIDTH / 2, CGRectGetMaxY(_hollowBkView.frame) + 30);
    [self.view addSubview:start];
}

- (void)newStyle {
    UIButton * start = [UIButton buttonWithType:UIButtonTypeCustom];
    [start setTitle:@"style-1" forState:UIControlStateNormal];
    [start setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [start addTarget:self action:@selector(gotoNext) forControlEvents:UIControlEventTouchUpInside];
    start.bounds = CGRectMake(0, 0, 80, 40);
    start.backgroundColor = [UIColor yellowColor];
    start.center = CGPointMake(SCREEN_WIDTH / 2 - 80, SCREEN_HEIGHT - 60);
    [self.view addSubview:start];
    
    UIButton * start2 = [UIButton buttonWithType:UIButtonTypeCustom];
    [start2 setTitle:@"style-2" forState:UIControlStateNormal];
    [start setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [start2 addTarget:self action:@selector(gotoNext2) forControlEvents:UIControlEventTouchUpInside];
    start2.bounds = CGRectMake(0, 0, 80, 40);
    start2.backgroundColor = [UIColor yellowColor];
    start2.center = CGPointMake(SCREEN_WIDTH / 2 + 80, SCREEN_HEIGHT - 60);
...
- (void)createProgressLabel
{
    ...
    _progressLabel.center = CGPointMake(SCREEN_WIDTH / 2, 50 + 64);
    ...
    _progressLabel2.center = CGPointMake(SCREEN_WIDTH / 2, 105 + 64);
    ...
    start.bounds = CGRectMake(0, 0, 80, 40);
    start.backgroundColor = [UIColor yellowColor];
    start.center = CGPointMake(SCREEN_WIDTH / 2 - 80, CGRectGetMaxY(_progressLabel2.frame) + 30);
    [self.view addSubview:start];
    ...
    start2.bounds = CGRectMake(0, 0, 80, 40);
    start2.backgroundColor = [UIColor yellowColor];
    start2.center = CGPointMake(SCREEN_WIDTH / 2 + 80, CGRectGetMaxY(_progressLabel2.frame) + 30);
    [self.view addSubview:start2];
}
